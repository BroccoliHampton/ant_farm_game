...
// showEventToast — inline to avoid circular dep with ui.js
function showEventToast(name,desc){ const t=document.getElementById('toast'); if(!t)return; t.innerHTML=`<div class='toast-name'>${name}</div><div class='toast-desc'>${desc}</div>`; t.classList.add('visible'); clearTimeout(t._timer); t._timer=setTimeout(()=>t.classList.remove('visible'),3500); }
